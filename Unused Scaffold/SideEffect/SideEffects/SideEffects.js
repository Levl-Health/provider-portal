import { Link, routes } from '@redwoodjs/router'
import { useMutation } from '@redwoodjs/web'
import { toast } from '@redwoodjs/web/toast'

import { QUERY } from 'src/components/SideEffect/SideEffectsCell'
import { truncate } from 'src/lib/formatters'

const DELETE_SIDE_EFFECT_MUTATION = gql`
  mutation DeleteSideEffectMutation($id: Int!) {
    deleteSideEffect(id: $id) {
      id
    }
  }
`

const SideEffectsList = ({ sideEffects }) => {
  const [deleteSideEffect] = useMutation(DELETE_SIDE_EFFECT_MUTATION, {
    onCompleted: () => {
      toast.success('SideEffect deleted')
    },
    onError: (error) => {
      toast.error(error.message)
    },
    // This refetches the query on the list page. Read more about other ways to
    // update the cache over here:
    // https://www.apollographql.com/docs/react/data/mutations/#making-all-other-cache-updates
    refetchQueries: [{ query: QUERY }],
    awaitRefetchQueries: true,
  })

  const onDeleteClick = (id) => {
    if (confirm('Are you sure you want to delete sideEffect ' + id + '?')) {
      deleteSideEffect({ variables: { id } })
    }
  }

  return (
    <div className="rw-segment rw-table-wrapper-responsive">
      <table className="rw-table">
        <thead>
          <tr>
            <th>Id</th>
            <th>Patient metrics id</th>
            <th>Patient id</th>
            <th>Title</th>
            <th>&nbsp;</th>
          </tr>
        </thead>
        <tbody>
          {sideEffects.map((sideEffect) => (
            <tr key={sideEffect.id}>
              <td>{truncate(sideEffect.id)}</td>
              <td>{truncate(sideEffect.patientMetricsId)}</td>
              <td>{truncate(sideEffect.patientId)}</td>
              <td>{truncate(sideEffect.title)}</td>
              <td>
                <nav className="rw-table-actions">
                  <Link
                    to={routes.sideEffect({ id: sideEffect.id })}
                    title={'Show sideEffect ' + sideEffect.id + ' detail'}
                    className="rw-button rw-button-small"
                  >
                    Show
                  </Link>
                  <Link
                    to={routes.editSideEffect({ id: sideEffect.id })}
                    title={'Edit sideEffect ' + sideEffect.id}
                    className="rw-button rw-button-small rw-button-blue"
                  >
                    Edit
                  </Link>
                  <button
                    type="button"
                    title={'Delete sideEffect ' + sideEffect.id}
                    className="rw-button rw-button-small rw-button-red"
                    onClick={() => onDeleteClick(sideEffect.id)}
                  >
                    Delete
                  </button>
                </nav>
              </td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  )
}

export default SideEffectsList
